...
public class XXHashPartitioner implements IPartitioner {
    ...
    // Modified getHash method to use XXHash
    private long[] getHash(ByteBuffer key) {
        long[] hash = new long[2];
        int seed = 0; // XXHash seed, can be modified as needed

        if (key.remaining() > 0) {
            hash[0] = factory.hash64().hash(key, key.position(), key.remaining(), seed);
            key.position(0); // Reset the position after hashing
        } else {
