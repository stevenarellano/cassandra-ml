...
public class CityHashPartitioner implements IPartitioner {
    ...
    public static final CityHashPartitioner instance = new CityHashPartitioner();
    ...
    // modified for use of city hash
    private long[] getHash(ByteBuffer key) {
        long[] hash = new long[2];
        int seed = 0; // CityHash seed, can be modified as needed
        ...
        if (key.remaining() > 0) {
            ...
            if (key.hasArray()) {
                ...
                // Create a new byte array and copy the data from ByteBuffer
                keyBytes = new byte[key.remaining()];
                key.mark(); // Mark the current position
                key.get(keyBytes);
                key.reset(); // Reset to the original position
            }

            // Use CityHash here to compute the hash of the key
            hash[0] = CityHash.cityHash64WithSeed(keyBytes, 0, keyBytes.length, seed);
        } else {
            hash[0] = 0L; // Define a constant hash for empty keys
        }
